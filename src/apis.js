...
export function signIn(username,password) {
    return request("http://localhost:8000/auth/token/login", {
...
export function register(username,password) {
    return request("http://localhost:8000/auth/users/", {
        data: {username,password},
        method: "POST",
    })
}
...
export function uploadImage(image) {
    ...
    return fetch("https://api.cloudinary.com/v1_1/musagor/image/upload", {
        method: "POST",
        body: formData,
    }).then((response) => {
        return response.json();
